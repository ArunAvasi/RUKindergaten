...
public class Classroom {
        ...
        for(int i=0; i<studentsSitting.length; i++)
        {
            for(int j=0; j<studentsSitting[0].length; j++)
            {
                if(studentsSitting[i][j] != null)
                {
                    ...
                    {
                        pointer.setStudent(studentsSitting[i][j]);
                        musicalChairs.setNext(pointer);
                    }
                    else{
                        pointer.setNext(new SNode(studentsSitting[i][j], null));
    ...
    public void moveStudentFromChairsToLine(int size) {
        ...
        while(musicalChairs != null && cnt<=num  ){
            if(cnt == num)
            {
                if(num == 0)
                {
                    removed = pointer.getStudent();
    ...
    public void insertByHeight(Student studentToInsert) {
        ...
        else if(studentToInsert.getHeight() <studentsInLine.getStudent().getHeight())
        {
            ...
            {
                if((studentToInsert.getHeight() < ptr.getNext().getStudent().getHeight())&&(studentToInsert.getHeight() > ptr.getStudent().getHeight()))
                {
                    ptr.setNext(new SNode(studentToInsert, ptr.getNext()));
                    return;
                }
                else if( (studentToInsert.getHeight() != ptr.getNext().getStudent().getHeight())&&(studentToInsert.getHeight() == ptr.getStudent().getHeight()))
                {
                    ptr.setNext(new SNode(studentToInsert, ptr.getNext()));
                    return;
                }
    ...
    public void eliminateLosingStudents() {
        ...
        int sizes = 1;
        SNode pointers = musicalChairs;
        while(pointers.getNext()!= musicalChairs)
        {
        ...
        while(sizes>1)
        {
            moveStudentFromChairsToLine(sizes);
            sizes--;
        }
    }
    ...
    public void seatMusicalChairsWinner () {
        ...
        {
            int i = 0;
            while (i < seatingLocation.length) {
                int j = 0;
                while (j < seatingLocation[0].length) {
                    if (seatingLocation[i][j] == true && !endd) {
                        stdnt = musicalChairs.getStudent();
                        studentsSitting[i][j] = stdnt;
                        musicalChairs = null;
